...
public final class Main extends Application {
    private static final String chess = "Шахматы";
    private final int size = 8;
    private final Logic logic = new Logic();

    private Rectangle buildRectangle(int x, int y, int size, boolean white) {
        Rectangle rect = new Rectangle();        rect.setX(x * size);
        rect.setY(y * size);
        rect.setHeight(size);
        rect.setWidth(size);
        if (white) {
    ...
    private Rectangle buildFigure(int x, int y, final int size, String image) {
        ...
        rect.setOnMouseReleased(
                new EventHandler<MouseEvent>() {
                    public void handle(MouseEvent event) {
                        try {
                            logic.move(
                                    Main.this.findBy(momento.getX(), momento.getY()),
                                    Main.this.findBy(event.getX(), event.getY()));
